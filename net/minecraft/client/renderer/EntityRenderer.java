...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            float reachDist = Main.instance.featureDirector.getFeatureByClass(Reach.class).isToggled() ? Reach.reachValue.getNumberValue() : 3;
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (flag) {
                ...
                if (Config.zoomMode) {
                    if (this.scopeMath < 5.0F) {
                        this.scopeMath += 2 * Feature.deltaTime();
                    }
                }
            } else {
                if (this.scopeMath > 1.0F) {
                    this.scopeMath -= 3 * Feature.deltaTime();
                }
                if (Config.zoomMode) {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            if (NoRender.hurtcam.getBoolValue() && Main.instance.featureDirector.getFeatureByClass(NoRender.class).isToggled()) {
                GlStateManager.rotate(-f * 0, 0.0F, 0.0F, 1.0F);
            } else {
                GlStateManager.rotate(-f * 14, 0.0F, 0.0F, 1.0F);
            }
            GlStateManager.rotate(f2, 0.0F, 1.0F, 0.0F);
        }
    }
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (raytraceresult != null) {
                        ...
                        if (NoRender.cameraClip.getBoolValue() && Main.instance.featureDirector.getFeatureByClass(NoRender.class).isToggled()) {
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                if (Main.instance.featureDirector.getFeatureByClass(NightMode.class).isToggled()) {
                    int color = NightMode.worldColor.getColorValue();
                    int r = color >> 16 & 0xFF;
                    int g = color >> 8 & 0xFF;
                    int b = color & 0xFF;
                    int a = color >> 24 & 0xFF;
    ...
    protected void renderRainSnow(float partialTicks) {
        ...
        float f5 = snow ? 1 : this.mc.world.getRainStrength(partialTicks);
                    ...
                    if ((biome.canRain() || biome.getEnableSnow()) || snow) {
                        ...
                        if (k2 != l2) {
                            ...
                            if ((world.getBiomeProvider().getTemperatureAtHeight(f1, j2) >= 0.15F) && !snow) {
                                ...
                                int red = (int) (((Weather.weatherColor.getColorValue() >> 16) & 255) / 100.f);
                                int green = (int) (((Weather.weatherColor.getColorValue() >> 8) & 255) / 100.f);
                                int blue = (int) ((Weather.weatherColor.getColorValue() & 255) / 100.f);
                                bufferbuilder.pos((double) l1 - d3 + 0.5D, (double) l2, (double) k1 - d4 + 0.5D)
                                        .tex(0.0D + d9, (double) k2 * 0.25D + d8 + d10).color(red, green, blue, f4)
                                        .lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 + d3 + 0.5D, (double) l2, (double) k1 + d4 + 0.5D)
                                        .tex(1.0D + d9, (double) k2 * 0.25D + d8 + d10).color(red, green, blue, f4)
                                        .lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 + d3 + 0.5D, (double) k2, (double) k1 + d4 + 0.5D)
                                        .tex(1.0D + d9, (double) l2 * 0.25D + d8 + d10).color(red, green, blue, f4)
                                        .lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 - d3 + 0.5D, (double) k2, (double) k1 - d4 + 0.5D)
                                        .tex(0.0D + d9, (double) l2 * 0.25D + d8 + d10).color(red, green, blue, f4)
                                        .lightmap(j4, k4).endVertex();
                            }
                        }
                    }
