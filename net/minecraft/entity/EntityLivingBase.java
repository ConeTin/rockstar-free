...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        if (Main.instance.featureDirector.getFeatureByClass(SwingAnimations.class).isToggled()) {
            final int speed = (int)SwingAnimations.smooth.getNumberValue();
    ...
    protected void jump2() {
    	this.motionY = (double) 0.22;

        if (this.isSprinting()) {
            float f = this.rotationYaw * 0.017453292F;
            this.motionX -= (double) (MathHelper.sin(f) * Flight.speed.getNumberValue());
            this.motionZ += (double) (MathHelper.cos(f) * Flight.speed.getNumberValue());
        }
    }
        ...
        if (!this.world.isRemote) {
            ...
            for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
                ...
                if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
                    ((WorldServer) this.world).getEntityTracker().sendToAllTrackingEntity(this,
                    ...
                    switch (entityequipmentslot.getSlotType()) {
                        case HAND:
                            this.handInventory.set(entityequipmentslot.getIndex(),
                                    itemstack1.func_190926_b() ? ItemStack.field_190927_a : itemstack1.copy());
                            break;
    ...
    public Vec3d getLook(float partialTicks) {
        if (!Main.instance.featureDirector.getFeatureByClass(KillAura.class).isToggled() &&
                this instanceof net.minecraft.client.entity.EntityPlayerSP)
    ...
    public boolean isActiveItemStackBlocking(final int ticks) {
        if (this.isHandActive() && !this.activeItemStack.isEmpty()) {
            final Item item = this.activeItemStack.getItem();
            return item.getItemUseAction(this.activeItemStack) == EnumAction.BLOCK && item.getMaxItemUseDuration(this.activeItemStack) - this.activeItemStackUseCount >= ticks;
        }
        return false;
    }
}
