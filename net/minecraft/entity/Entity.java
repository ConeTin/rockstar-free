...
public abstract class Entity implements ICommandSender
{
    ...
    public float getDistanceToPoint(Point entityIn)
    {
        float f = (float)(this.posX - entityIn.getX());
        float f1 = (float)(this.posY - entityIn.getY());
        float f2 = (float)(this.posZ - entityIn.getZ());
        return MathHelper.sqrt(f * f + f1 * f1 + f2 * f2);
    }
    ...
    public float getCollisionBorderSize() {
        if(Main.instance.featureDirector.getFeatureByClass(HitBox.class).isToggled()) {
            return (float) (0.0F + HitBox.hitboxsize.getNumberValue());
        } else {
            return (float) (0.0F);
        }
    }
